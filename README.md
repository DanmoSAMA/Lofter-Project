:warning:如要正常查看该项目，请下载node.js和mongoDB数据库，并完成相应的配置。

# Lofter-Project

 对Lofter的复刻

> 冰岩作坊2021春招实习任务 T1 铜人女的天堂
...
* 修改了index.html，做出了两个tag的页面，为搜索做准备。
* 修改主页最上方的标签搜索框，解决了:focus伪类带来的一些弊端，现在点击标签可以自动填入表单
* 简易搜索功能：输入“逆转裁判”或“天元突破”，可以跳转到对应的页面
* 调整了"展开"和"热度、评论、分享、推荐"的位置
* 给发评论的框框添加了左上角的三角形，觉得clip-path实在是香
* 解决了几个小bug
* 了解到style会频发触发重排和重绘，有性能问题，将js中一部分style修改样式的代码，改为了修改类名
* 把四空格缩进的CSS代码格式化为两空格
...
* 跳转到其他页面时node会崩，重启了数据库和node后恢复正常
* 想做点击标签自动填入，结果:focus伪类会使得我在点到标签之前，先让下拉框消失，从而导致onclick事件不能触发。修改JS后解决该问题。

### 计划

* 学姐建议配置路由，把绝对路径改成相对路径，我考虑到这个工程量可能略大（之前才花时间把相对路径改为绝对路径...），而且该程序是跑在自己的电脑上，并未出现什么问题，所以把它的优先度降低了；并且我是通过/public 这个条件去请求资源的，不知道怎么路由...先放着吧...

### 心得
* 相对路径和绝对路径一开始就想好
* 一开始没考虑到表单元素外边要套< form>，后来改得头皮发麻



# 项目心得

* 感觉写html和css来得更快了，虽然还是犯低级错误（忘了改line-height，写到错误的文件里，忘记引入js/css结果苦恼为什么没有现象）
* js也写的更熟练一点，不再是全篇let，🐻学长建议不要通过style来修改属性，~~但是我考虑到有时候只改一两个样式，所以还是用了style~~，在🦊的建议下，我将部分style改为了类名
* node感觉入门了，而且是第一次操作数据库，成就感upup
* 希望以后能接触更多的项目，锻炼自己的代码能力，提高代码质量
